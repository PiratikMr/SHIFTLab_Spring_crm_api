...
Для запуска приложения и базы данных используйте предоставленный `docker-compose.yml`.
...
| **PostgreSQL DB** | `5444` | `localhost:5444` |
...
| **DELETE** | `/sellers/{id}` | **Удаление продавца** по ID. | **Параметры пути:** <br> - `{id}` (bigint) ID продавца |
...
| **GET** | `/transactions/seller/{sellerId}` | **Получение списка транзакций** продавца. | **Параметры пути:** <br> - `{sellerId}` (bigint) <br><br> **Параметры запроса:** <br> - `page` (int, default: 0) Номер страницы <br> - `perPage` (int, default: 10) Количество элементов |
| **GET** | `/transactions` | **Получение списка всех транзакций**. | **Параметры запроса:** <br> - `page` (int, default: 0) Номер страницы <br> - `perPage` (int, default: 10) Количество элекментов |
| **PUT** | `/transactions/{id}` | **Обновление информации о транзакции**. | **Параметры пути:** <br> - `{id}` (bigint) ID продавца <br><br> **Схема тела запроса:** <br> - `amount` (bigDecimal) Сумма транзакции <br> - `paymentType` (string) Тип оплаты (`'CASH'`, `'CARD'` или `'TRANSFER'`) |
...
| Метод | Путь | Описание | Параметры |
| :--- | :--- | :--- | :--- |
| **GET** | `/analytics/most-productive-seller/{periodType}` | **Самый продуктивный продавец** за период. | **Параметры пути:** <br> - `{periodType}` (`DAY`, `MONTH`, `QUARTER`, `YEAR`) <br> |
